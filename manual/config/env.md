...
- `INIT_SS_ADMIN_USER` and `INIT_SS_ADMIN_PASSWORD`: SeaSearch administrator credentials, used only on first deployment. They default to the Seafile administrator credentials in the Docker Pro template.
- `SS_MAX_OBJ_CACHE_SIZE`: Maximum SeaSearch object-cache size, default is `10GB`.
- `SS_LOG_TO_STDOUT`, `SS_LOG_OUTPUT`, and `SS_LOG_LEVEL`: SeaSearch log settings.

For the complete SeaSearch environment-variable reference, see the [SeaSearch manual](https://seasearch-manual.seacloud-labs.ai/latest/config/).
